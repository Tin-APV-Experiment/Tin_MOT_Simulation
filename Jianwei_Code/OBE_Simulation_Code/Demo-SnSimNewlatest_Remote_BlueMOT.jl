...
bGradReal = 75# in units Gauss/cm.  if "Static", this becomes the static field, in Gauss\
...
userSpeeds = [-3,-2.5,-2,-1.5,-1,-.75,-.5,-.25,-.1,.1,.25,.5,.75,1,1.5,2,2.5,3].*0.05;#BlueMOT Setting/ FreeSpace_Sub Doppler Cooling Setting
...
laserEnergy = [3.0, -2.5]*1.0;## Blue MOT Setting
...
if saveData==1
    ...
    folderString = string(parent_dir,"/SnMOT_Data/Blue_MOT/Regular_BlueMOT/",date_str,"/",saveDataFolderTag,"BGradGPerCM", bGradReal,time_str,"Trail",numTrialsPerValueSet)
    if useRandPhase == 1
