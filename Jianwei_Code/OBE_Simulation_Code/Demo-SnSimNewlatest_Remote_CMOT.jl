...
bGradReal = 25# in units Gauss/cm.  if "Static", this becomes the static field, in Gauss\
...
#=
#for capture redMOT
#displacementsInMM = [1,2,3,5,7,9,11,14,17]; # RedMOT Setting
displacementsInMM = [0.05,0.1,0.3,0.5,1,2,3,5,7,9,11,14]; # RedMOt Setting
#displacementsInMM = [1,2,3,5,7,9,11];
#userSpeeds = [-8,-6.5,-5,-3.5,-3,-2.5,-2,-1.5,-1,-.6,-.4,-.2,-.1,.1,.2,.4,.6,1,1.5,2,2.5,3,3.5,5,6.5,8]; 
#userSpeeds = [-4,-3.5,-3,-2.5,-2,-1.5,-1,-.6,-.4,-.2,-.1,.1,.2,.4,.6,1,1.5,2,2.5,3,3.5,4]; #Compression Stage 1
userSpeeds = [-5,-3.5,-3,-2.5,-2,-1.5,-1,-.6,-.4,-.2,-.1,.1,.2,.4,.6,1,1.5,2,2.5,3,3.5,5]; # RedMOT
=#
...
#displacementsInMM = [0.5,1,1.5,2.5,3.5,4.5,5.5];  # Compression Stage 1
#displacementsInMM = [0.25,0.5,0.75,1.25,1.75,2.25,2.75];
#displacementsInMM = [0.01,0.03,0.08,0.15,0.3,0.5,0.9,1.5,3.0,5.0]; # Compression Stage 2 / 3
#displacementsInMM = [0.05,0.15,0.3,0.5,0.9,1.5,3.0];
#displacementsInMM = [0.1,0.2,0.3,0.5,0.9,1.5,3.0];
#displacementsInMM = [0.15,0.3,0.45,0.75,1.15,1.5,3.0];
#userSpeeds = [-4,-3,-2.5,-2,-1.5,-1,-.75,-.5,-.25,.25,.5,.75,1,1.5,2,2.5,3,4].*0.4; Compression Stage 1
userSpeeds = [-4,-3,-2.5,-2,-1.5,-1,-.75,-.5,-.25,.25,.5,.75,1,1.5,2,2.5,3,4].*0.2; # Compression Stage 2 / 3

#format: [red, blue]
s0 = [1.5,0.3].*0.03;
laserEnergy = [-1,0.5]*0.5;
polSign = [-1,1];
...
#userSpeeds = [-3,-2.5,-2,-1.5,-1,-.75,-.5,-.25,.25,.5,.75,1,1.5,2,2.5,3]; ## Setting
#userSpeeds = [-3,-2.4,-1.9,-1.4,-.9,-.5,-.3,-.1,.1,.3,.5,.9,1.4,1.9,2.4,3].*0.05;#BlueMOT Setting
#userSpeeds = [-3,-2.5,-2,-1.5,-1,-.75,-.5,-.25,.25,.5,.75,1,1.5,2,2.5,3].*0.02;#FreeSpace_Sub Doppler Cooling Setting

#format: [red, blue]
#s0 = [1.5, 0.3]*1;## Blue MOT Seeting
#s0 = [0.3, 0.3]*1;## Lambda-Setting (Zero-Bfield)
#detu=[1,1]*0.5;
#laserEnergy = [-3.5, -3.5]*-1.0;## Lambda-Setting (Zero-Bfield)
#laserEnergy = [-1.0, 0.5]*1.0;## Blue MOT Setting
#polSign = [-1,1]*1; ## Blue/Lambda Seeting
...
if saveData==1
    ...
    folderString = string(parent_dir,"/SnMOT_Data/CMOT/",date_str,"/",saveDataFolderTag,"BGradGPerCM", bGradReal,time_str,"Trail",numTrialsPerValueSet)
    if useRandPhase == 1
