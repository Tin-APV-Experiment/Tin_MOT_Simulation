...
LaserCondition = " (2.5 0.5) (3.0 -2.5) (-1,1)";
...
%pos={'0.01','0.03','0.05','0.1','0.3','0.6','1.0'}; 
...
%make heat map
vsForHeatMap = [min(vels):.01:max(vels)];
xsForHeatMap = [min(sortedPos):.01:max(sortedPos)];
...
if SavePlot==1
    ...
    output_folder_Haetplot = strcat('SnPapertestPlot','/HeatPlot/','/',Date,'/');
...
if SavePlot==1
    output_folder_pos = strcat('SnPapertestPlot','/AccelerationVsPosition/','/',Date,'/');
...
if SavePlot==1
    output_folder_vel = strcat('SnPapertestPlot','/AccelerationVsVelocity/','/',Date,'/');
...
if simTrapDynamics == 1
    ...
    maxTime = 10; %100 ms time evolution after capture in MOT
    initPos = 0; %assume start at center of t rap
    ...
    %write to csv
    if SavePlot==1
        output_folder_traj = strcat('SnPapertestPlot','/particle_trajectory_MOT/','/',Date,'/');
