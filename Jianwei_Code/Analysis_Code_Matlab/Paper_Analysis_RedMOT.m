...
bfield_grad = "25"; %b-field gradient used
data_timestamp = "20250415_1101";
...
LaserCondition = " (0.5 0.5) (3.0 -2.5) (1 -1)";
...
pos = {'0.05','0.1','0.3','0.5','1.0', '2.0', '3.0', '5.0', '7.0', '9.0', '11.0', '14.0'};
%pos={'0.5','1.0','1.5','2.0','2.5','3.0','5.0'};
%pos = {'1', '2', '3', '5', '7', '9', '11'};
%compressed MOT pos settings
%pos = {'0.5', '1.0', '1.5', '2.5', '3.5', '4.5', '5.5'};
...
plotHandles = gobjects(round(currV), 1); % Preallocate for better performance

% Initialize a cell array to store legend labels
legendLabels = cell(round(currV), 1);

% --- Initialize Data Storage ---
% Create an empty array to store all trajectory data.
% Each row will have [position, velocity, initial velocity (v0)].
allData = [];

% Define the range of initial velocities:
% Starting from round(currV) down to 1 with a step of -3.
capvel_up=ceil(capVel);
velocities = [capvel_up,capVel,capVel-15,5];
%velocities = round(currV):-3:1;
% --- Loop Through Each Initial Velocity ---
for idx = 1:length(velocities)
    v = velocities(idx);  % Current initial velocity
    vround = round(v);
    
    [ts2, ps2] = ode23(diffEqVals, [0 50], [min(sortedPos); v]);%Solving ode to get trajectaries
    
    % Plot the entire trajectory and store the plot handle
    plotHandles(vround) = plot(ps2(:, 1), ps2(:, 2), 'LineWidth',2);
    
    % Store the legend label for the current plot
    legendLabels{vround} = ['v_0 = ' num2str(v) ' m/s'];
    
    % Create a column vector with the same number of rows as ps2,
    % containing the current initial velocity v.
    v_col = v * ones(size(ps2, 1), 1);
    % Concatenate the solution (ps2) and the v_col column.
    % The resulting matrix has three columns: position, velocity, and initial velocity (v0).
    trajectory = [ps2, v_col];
    
    % Append the current trajectory data to the overall data matrix.
    allData = [allData; trajectory]; %
end
...
% Create the legend
legend(plotHandles(round(velocities)), legendLabels(round(velocities)), 'Location', 'best');
...
%Save Data
% Convert the numerical matrix to a table with column headers.
T = array2table(allData, 'VariableNames', {'position', 'velocity', 'v0'});

% Write the table to a CSV file named 'all_trajectories.csv'.
writetable(T, strcat('SnMOTPaper/Capvel_trajectories', data_timestamp, '.csv'));
...
grid on;
grid minor; 
title('Sn BlueMOT Dynamics Heat Map')
...
if simTrapDynamics == 1
    ...
    maxTime = 30; %100 ms time evolution after capture in MOT
    initPos = -0.0; %assume start at center of t rap
    initVel = 0.2; %assume start with zero velocity
    gam = molecule.gam;
    kA = molecule.kA;
    mass = molecule.mass;
    hbar = 1.05e-34;
    ...
    %write to csv
    output_folder = 'SnMOTPaper/';
    writematrix([temp, sigma], fullfile(output_folder, strcat('MOTTempAndSigma_', data_timestamp, '.csv')));
    writematrix(ps2, strcat('SnMOTPaper/MOTTrajectoryNoScatter', data_timestamp, '.csv'));
    writematrix([r; v]', strcat('SnMOTPaper/MOTTrajectoryYesScatter', data_timestamp, '.csv'));
